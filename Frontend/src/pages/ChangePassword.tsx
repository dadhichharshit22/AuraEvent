...
const ChangePassword: React.FC<ChangePasswordProps> = ({ email }) => {
  ...
  const handleChangePassword = async () => {
    try {
      await AuthService.changePassword(email, formData.password, formData.confirmPassword);
  ...
  return (
    <div className="flex gap-2 justify-between h-screen text-black">
      ...
      <div className="w-1/2 h-full overflow-hidden">
        <img src={ilus} alt="Illustration" className="object-cover w-full h-full rounded-r-3xl" />
      </div>
