...
app.post('/capturePayment', (req, res) => paymentService.capturePayment(req, res));
app.post('/verifyPayment', (req, res) => paymentService.verifyPayment(req, res));
...
describe('PaymentService', () => {
  ...
  describe('verifyPayment', () => {
    it('should return 400 if payment verification fails', async () => {
      // Fix the spy by using the correct instance and method
      ...
      expect(response.body).toEqual({ success: false, message: 'Payment verification failed' });
    });
    ...
    it('should return 200 if payment is verified and attendee is added', async () => {
      ...
      (EventService.addAttendee as jest.Mock).mockResolvedValue({ success: true });
      ...
      expect(response.body).toEqual({ success: true, message: 'Payment Verified' });
    });
  });
});
