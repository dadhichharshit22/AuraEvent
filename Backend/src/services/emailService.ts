...
export class EmailService {
  ...
  // Sends an email to multiple User.
    ...
    await Promise.all(
      recipientEmails.map((recipientEmail) =>
        this.sendEmail(
          recipientEmail,
          subject,
          contentGenerator(eventTitle, eventDate, eventDescription)
        )
      )
    );
  }
  ...
  async sendWelcomeEmail(
    ...
    await this.sendEmail(
      ...
      userRegistrationTemplate(userName)
    );
  }
  ...
  async notifyEventCreation(
    ...
    await this.notifyEvent(
      recipientEmail,
      "Event Created",
      eventNotificationTemplate(eventTitle, eventDate, eventDescription)
    );
  }
  ...
  async notifyNewEvent(
    ...
    await this.notifyEvent(
      ...
      newEventNotification(eventTitle, eventDate, eventDescription)
    );
  }
  ...
  async confirmRegistration(
    ...
    await this.notifyEvent(
      ...
      registrationEmailTemplate(eventTitle, eventDate)
    );
  }

  // Confirms an event unregistration to the user.

  async confirmUnregistration(
    recipientEmail: string,
    eventTitle: string,
    eventDate: string
  ): Promise<void> {
    await this.notifyEvent(
      ...
      unregistrationEmailTemplate(eventTitle, eventDate)
    );
  }
  ...
  async sendPaymentSuccessEmail(
    ...
    const content = paymentSuccessTemplate(
      eventTitle,
      amount,
      transactionId,
      paymentDate
